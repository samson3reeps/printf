...
int write_num(int ind, char buffer[], int flags, int width, int precision, int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == ' ')/* Assign extra character to the left of buffer */
		{
			if (extra_c)
				buffer[--ind] = extra_c;
			return (write(1, &buffer[1], i - 1) + write(1, &buffer[ind], length));
		}
		else if (!(flags & F_MINUS) && padd == '0')/* Assign extra character to the left of padding */
		{
			if (extra_c)
...
int write_unsgnd(int is_negative, int ind, char buffer[], int flags, int width, int precision, int size)
{
	...
	if (width > length)
	{
		...
		else /* Assign extra character to the left of buffer [padd > buffer] */
		{
